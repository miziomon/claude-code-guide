# Practical Guide to Claude Code CLI

> **Version 4.30 — May 2026**
> Verified on **Claude Code v2.1.123** · Licensed [CC BY-SA 4.0](https://creativecommons.org/licenses/by-sa/4.0/)

A hands-on guide covering installation, workflow, prompt engineering, and best practices for Claude Code.

---

## Table of Contents

| # | Chapter | Description |
|---|---------|-------------|
| — | [Foreword](00-foreword.md) | An honest assessment by the model the book is about |
| — | [Preface](00-preface.md) | Why this guide exists and how to use it |
| 1 | [What is Claude Code](01-what-is-claude-code.md) | Architecture, positioning, cost/benefit |
| 2 | [Installation and setup](02-installation.md) | First run, authentication, IDE integration |
| 3 | [The first end-to-end project](03-first-project.md) | A real project from scratch |
| 4 | [Essential commands and shortcuts](04-commands.md) | The commands you'll use every day |
| 5 | [Plan Mode](05-plan-mode.md) | Think before you write |
| 6 | [Prompt engineering](06-prompt-engineering.md) | Writing effective prompts for agentic tools |
| 7 | [Persistent memory](07-memory.md) | CLAUDE.md and Auto Memory |
| 8 | [Context management](08-context.md) | Keeping the context window clean |
| 9 | [Security, permissions, and guardrails](09-security.md) | Permission model, safe operation, and guardrails |
| 10 | [Skills](10-skills.md) | The extension mechanism |
| 11 | [MCP](11-mcp.md) | Integrating external services · [Example: wordpress-publisher-mcp](../examples/wordpress-publisher-mcp/) |
| 12 | [Subagents](12-subagents.md) | Orchestrating specialized work |
| 13 | [Hooks](13-hooks.md) | Automating Claude Code's lifecycle |
| 14 | [Plugins](14-plugins.md) | Distributable packages |
| 15 | [Advanced workflows](15-advanced-workflows.md) | Tips and advanced patterns |
| 16 | [Conclusions](16-conclusions.md) | Why CLI and not just chat |
| A | [Glossary](appendix-a-glossary.md) | Key terms and definitions |
| B | [Sources](appendix-b-sources.md) | Official references |
| — | [About the Author](about-the-author.md) | |

---

> **Full source:** [claude-code-guide-en.md](../claude-code-guide-en.md)
> **Italian version:** [../it/README.md](../it/README.md)
